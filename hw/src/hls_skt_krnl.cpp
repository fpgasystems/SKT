...
using T_AXIN0 = ap_axiu<N0*32, 0, 0, 0>;
using T_AXI64 = ap_axiu<64, 0, 0, 0>;
...
#pragma HLS stream variable=src_buf depth=49152 // 32768
#pragma HLS bind_storage variable=src_buf type=fifo impl=uram
	map(src, src_buf, [](T_AXIN0 const& x) -> flit_v_t<ap_uint<N0*32>> {
		...
		return  flit_v_t<ap_uint<N0*32>>{ x.last, x.data };
	});
